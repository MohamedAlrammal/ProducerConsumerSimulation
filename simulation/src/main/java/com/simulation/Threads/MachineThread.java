    ...
    public void run() {
        ...
        while(true){
            ...
            if(check) {
                ...
                observerClient.updatePreQueues(MoQueues,updatesQueue);
                latch.countDown();
                MachineClient machineClient =new MachineClient();
                machineClient.UpdateStateMachines(machine,true,updatesQueue);
                latch.countDown();
                MoQueues.clear();
                ...
                } machineClient.UpdateStateMachines(machine,false,updatesQueue);
                observerClient.updateFollQueues(MoQueues2,updatesQueue);
                latch.countDown();
                MoQueues2.clear();
                try {
                    Thread.sleep(1000);
